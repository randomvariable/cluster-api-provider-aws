...
var _ = ginkgo.Context("[unmanaged] [functional]", func() {
	...
	// TODO @randomvariable: This test is frequently failing, so disabling for now.
	// Should finish kubetest2 and test external CPI.
	ginkgo.PDescribe("Cluster name validations and CPI", func() {
		nginxStatefulsetInfo := getStatefulSetInfo()
		ginkgo.It("should create a cluster, AWS load balancer, and volume", func() {
			specName := "functional-test-cpi"
			requiredResources := &shared.TestResource{EC2: 2, IGW: 1, NGW: 1, VPC: 1, ClassicLB: 2, EIP: 3}
			requiredResources.WriteRequestedResources(e2eCtx, specName)
			namespace := shared.SetupSpecNamespace(ctx, specName, e2eCtx)
			defer shared.DumpSpecResourcesAndCleanup(ctx, "", namespace, e2eCtx)
			Expect(shared.AcquireResources(requiredResources, config.GinkgoConfig.ParallelNode, flock.New(shared.ResourceQuotaFilePath))).To(Succeed())
			...
			ginkgo.By("Creating cluster with name starting with 'sg-', having '.' and more than 22 characters")
			...
			ginkgo.By("Waiting for worker nodes to be in Running phase")
			...
			ginkgo.By("Verifying whether provisioned LB deleted")
			verifyElbExists(elbName, false)

			ginkgo.By("Verifying dynamically provisioned volumes retention")
			verifyVolumesExists(awsVolIds)
	...
	ginkgo.Describe("Multiple workload clusters", func() {
