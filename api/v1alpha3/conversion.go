...
func (r *AWSCluster) ConvertTo(dstRaw conversion.Hub) error {
	...
	if restored.Status.Bastion != nil {
		if restored.Status.Bastion.RootVolume != nil && restored.Status.Bastion.RootVolume.Encrypted == nil {
			dst.Status.Bastion.RootVolume.Encrypted = nil
		}

		for i := range restored.Status.Bastion.NonRootVolumes {
			if restored.Status.Bastion.NonRootVolumes[i].Encrypted == nil {
				dst.Status.Bastion.NonRootVolumes[i].Encrypted = nil
			}
		}
	}
	return nil
...
func restoreRootVolume(restored, dst *v1alpha4.Volume) {
	...
	if dst.Encrypted == true || restored == nil {
		return
	}
...
func restoreAWSMachineSpec(restored, dst *v1alpha4.AWSMachineSpec, src *AWSMachineSpec) {

	restoreRootVolume(restored.RootVolume, dst.RootVolume)

	for i := range restored.NonRootVolumes {
		// what if len(restored.NonRootVolumes) = 5 && dst.NonRootVolumes = 3
		// what happens at dst.NonRootVolumes[4]. Going to be NPE
		if restored.NonRootVolumes[i].Encrypted == nil {
			if len(dst.NonRootVolumes) <= i {
				dst.NonRootVolumes = append(dst.NonRootVolumes, restored.NonRootVolumes[i])
			} else {
				dst.NonRootVolumes[i].Encrypted = nil
			}
		}
	}
}
...
func (r *AWSMachine) ConvertTo(dstRaw conversion.Hub) error {
	...
	if restored.Spec.RootVolume != nil && restored.Spec.RootVolume.Encrypted == nil {
		dst.Spec.RootVolume.Encrypted = nil
	}

	for i := range restored.Spec.NonRootVolumes {
		if restored.Spec.NonRootVolumes[i].Encrypted == nil {
			dst.Spec.NonRootVolumes[i].Encrypted = nil
		}
	}
	return nil
}
...
func (r *AWSMachineTemplate) ConvertTo(dstRaw conversion.Hub) error {
	...
	if restored.Spec.Template.Spec.RootVolume != nil && restored.Spec.Template.Spec.RootVolume.Encrypted == nil {
		dst.Spec.Template.Spec.RootVolume.Encrypted = nil
	}

	for i := range restored.Spec.Template.Spec.NonRootVolumes {
		if restored.Spec.Template.Spec.NonRootVolumes[i].Encrypted == nil {
			dst.Spec.Template.Spec.NonRootVolumes[i].Encrypted = nil
		}
	}
	return nil
}
