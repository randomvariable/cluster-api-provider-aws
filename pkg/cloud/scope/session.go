...
func resolveSession(input resolveSessionInput) (*session.Session, error) {
	log := input.Logger.WithName("identity")
	log.V(4).Info("Creating an AWS Session")
	s, ok := sessionCache.Load(input.Region)
	if ok {
		return s.(*session.Session), nil
	}

	resolver := func(service, region string, optFns ...func(*endpoints.Options)) (endpoints.ResolvedEndpoint, error) {
		for _, s := range input.Endpoints {
	...
	awsConfig := aws.NewConfig().WithRegion(input.Region).WithEndpointResolver(resolver)
...
type buildProvidersInput struct {
	Providers    []AWSRolePrincipalTypeProvider
	Client       client.Client
...
func buildProviders(ctx context.Context, input buildProvidersInput) ([]AWSPrincipalTypeProvider, error) {
	if input.PrincipalRef == nil {
		log.V(4).Info("AWSCluster does not have a PrincipalRef specified")
		return providers, nil
	}

	var provider AWSPrincipalTypeProvider
	principalObjectKey := client.ObjectKey{Name: input.Reference.Name, Namespace: input.Reference.Namespace}
